    ...
    if not f_df.empty:
        ...
        with st.expander("Delete Vehicle"):
            if st.button("Confirm Delete"):
                ...
                new_f.to_csv(FLEET, index=False); st.rerun()
    with st.expander("Add Vehicle"):
        y = st.selectbox("Year", range(2027, 1990, -1)); ma = st.text_input("Make"); mo = st.text_input("Model")
        ct = st.radio("Type", ["Car/SUV", "Truck", "Motorcycle"])
        if st.button("Save Vehicle"):
            pd.concat([pd.read_csv(FLEET), pd.DataFrame([{"Year": y, "Make": ma, "Model": mo, "Cat": ct}])], ignore_index=True).to_csv(FLEET, index=False); st.rerun()

# --- 5. MAIN ---
st.title("📟 ANTONINO'S GARAGE HUB")
if not active_v: st.info("Please add a vehicle to begin."); st.stop()
...
with col1:
    ...
    if mode == "Oil Change":
        if active_cat == "Motorcycle":
            ...
            o_grade, o_liters, o_filter = st.text_input("Oil Grade"), st.text_input("Liters"), st.text_input("Filter #")
            entry["Oil_M"] = f"{o_grade} ({o_type})"
            entry["Notes"] = f"{o_liters}L | Filter: {o_filter} | {st.text_area('Notes')}"

    elif mode == "Tires":
        st.markdown("### Tires & PSI")
        ft1, ft2 = st.columns([2, 1]); f_s = ft1.text_input("Front Size"); f_p = ft2.text_input("Front PSI")
        rt1, rt2 = st.columns([2, 1]); r_s = rt1.text_input("Rear Size"); r_p = rt2.text_input("Rear PSI")
        ...
        entry["Type"], entry["Notes"] = f"Repair: {rep_cat}", st.text_area("Work Details")

    elif mode == "Diagnostic":
        st.markdown("### ⚡ Diagnostic Scan")
        d1, d2 = st.columns(2); dtc = d1.text_input("DTC"); abs_c = d1.text_input("ABS"); srs = d2.text_input("SRS"); oth = d2.text_input("Body")
        ...
        entry["Type"], entry["Bulbs"], entry["Notes"] = f"Lighting: {b_l}", st.text_input("Bulb Spec"), st.text_area("Replacement Notes")
    ...
    if not h_df.empty:
        ...
        with st.expander("📝 Edit History"):
            if not v_h.empty:
                ...
                if st.button("PURGE RECORD"): h_df.drop(sel).to_csv(LOG, index=False); st.rerun()
