...
LOG = "maintenance_log.csv"
FLEET = "fleet_database.csv"
# Fixed the unterminated string error from your photo
...
with col1:
    ...
    if mode == "Oil Change":
        if active_cat == "Motorcycle":
            ...
            # Fixed the multi-command syntax from your photo
            with c1:
                entry["Oil_M"] = st.text_input("Engine Oil", "20W-50")
            with c2:
        ...
        diag_notes = st.text_area('Live Data / Symptoms')
        entry["Notes"] = f"DTC:{dtc} | SRS:{srs} | ABS:{abs_c} | Body:{oth} | {diag_notes}"

    elif mode == "Bulbs":
        st.markdown("### 💡 Lighting")
        b_l = st.selectbox("Location", [
            "Low/High Beam", "Fog Lights", "Turn/Marker", 
            "License Plate", "Tail/Brake", "Reverse", 
            "Interior", "Side Marker", "Custom"
        ])
    ...
    if st.button("💾 SAVE RECORD"):
        ...
        pd.concat([current_log, new_row], ignore_index=True).to_csv(LOG, index=False)
        st.rerun()
    ...
    if not h_df.empty:
        ...
        with st.expander("📝 Edit History"):
            if not v_h.empty:
                ...
                sel = st.selectbox("Select record to delete", v_h.index, format_func=lambda x: v_h.loc[x, 'Display'])
